...
- (void)initMiniMap
{
    BMKMapView *mapView = [[BMKMapView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 180.5)];
    [self.mapContent addSubview:mapView];
    self.mapView = mapView;
    self.mapView.zoomLevel = 18;
    self.mapView.centerCoordinate = CLLocationCoordinate2DMake([self.parkEntity.mapLat doubleValue], [self.parkEntity.mapLng doubleValue]);
    self.mapView.userInteractionEnabled = NO;
    BMKPointAnnotation *currAnno = [[BMKPointAnnotation alloc] init];
    currAnno.coordinate = CLLocationCoordinate2DMake([self.parkEntity.mapLat doubleValue], [self.parkEntity.mapLng doubleValue]);
...
//            cell.commonTitle.text = [NSString stringWithFormat:@"%@ %@ 回复 %@", commentDic[@"date"], commentDic[@"userName"], commentDic[@"replyTo"]];
        ...
        if ([commentDic[@"userName"] isEqualToString:[DataManager getUserName]]) {
            [cell.openButton setTitle:@"更多" forState:UIControlStateNormal];
        }else{
            [cell.openButton setTitle:@"回复" forState:UIControlStateNormal];
        }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row == 0) {
        ...
        CGSize size = [[NSString stringWithFormat:@"    %@", self.parkEntity.parkDescription] stringSizeConstrainedToSize:CGSizeMake([UIScreen mainScreen].bounds.size.width-95, 1000) WithFont:[UIFont systemFontOfSize:14]];
        ...
        if (self.parkTagsView.contentSize.height < 44) {
            return 44;
        }else if (self.parkTagsView.contentSize.height > 110){
            return 110;
        }else{
            return self.parkTagsView.contentSize.height + 10;
        }
    }else if (indexPath.row == 4){
        return 413;
    }else{
...
- (void)parkCommentOpen:(UIButton *)button
{
    if ([button.titleLabel.text isEqualToString:@"回复"]) {
        [self parkCommentReply:button];
    }else if([button.titleLabel.text isEqualToString:@"更多"]){
        [button becomeFirstResponder];
        UIMenuController *menu = [UIMenuController sharedMenuController];
        menu.tag = button.tag;
        UIMenuItem *huifu = [[UIMenuItem alloc] initWithTitle:@"回复" action:@selector(parkCommentReply:)];
        UIMenuItem *edit = [[UIMenuItem alloc] initWithTitle:@"修改" action:@selector(parkCommentEdit:)];
        UIMenuItem *delete = [[UIMenuItem alloc] initWithTitle:@"删除" action:@selector(parkCommentDelete:)];
        menu.menuItems = @[huifu,edit,delete];
        [menu setTargetRect:button.frame inView:button.superview];
...
- (void)parkCommentReply:(UIButton *)button
{
    self.isRecover = YES;
...
- (void)parkCommentEdit:(UIButton *)button
{
    ...
    NSDictionary *commentDic = self.comments[button.tag-openBTag];
    self.currReplyName = commentDic[@"userName"];
    self.commentCoverField.text = commentDic[@"content"];
    self.currEditComment = commentDic;
    self.recoverDimView.alpha = 1;
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(keyboardWillChangeFrame:) name:UIKeyboardWillChangeFrameNotification object:nil];
    [self.commentCoverField becomeFirstResponder];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([UIScreen mainScreen].bounds.size.width < 330)
    {
        ...
        if (size.width + 2 < 45) {
            ...
            return CGSizeMake(size.width+12, 25);
        }
...
{
    static NSString *annoReuseID = @"theID";
...
@dynamic tag;
static NSInteger tagFlag;

- (void)setTag:(NSInteger)tag
{
    NSNumber *t = @(tag);
    objc_setAssociatedObject(self, &tagFlag, t, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSInteger)tag
{
    NSNumber *t = objc_getAssociatedObject(self, &tagFlag);
