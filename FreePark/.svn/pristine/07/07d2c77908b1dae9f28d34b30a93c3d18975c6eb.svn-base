...
+ (instancetype)instanceFromNib
{
    ...
    view.changeTags.layer.cornerRadius = 4;
    view.changeTags.layer.borderColor = [UIColor grayColor].CGColor;
    view.changeTags.layer.borderWidth = 1;
    ...
    if (indexPath.section == 0) {
        ...
        for (ParkTag *parkTag in self.pickedTags) {
            if ([parkTag.tagName isEqualToString:((ParkTag *)self.readyPickTags[indexPath.row]).tagName]) {
                [Utils showToastWithMessage:@"您已经选过这个标签了"];
                return;
            }
        }
